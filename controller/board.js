...
var boardController = (function () {
  ...
  const addBoardUser = function (idx, users, boardId, self) {
    ...
    let insertBoardUserQuery =
      "INSERT INTO board_user (user_id,board_id,role_id)" +
      "VALUES('" +
      users[idx].user_id +
      "','" +
      boardId +
  ...
  const deleteBoardUser = function (idx, users, boardId, self) {
    ...
    let insertBoardUserQuery =
      "DELETE FROM board_user WHERE board_id= '" +
      boardId +
      "' AND user_id = '" +
      users[idx].user_id +
      "'";
  ...
  board.prototype.checkUserIsAuthenticated = async function (boardId, userId) {
    let query =
      "SELECT (SELECT user_id FROM board WHERE id='" +
      boardId +
      "' && user_id='" +
      userId +
      "') as board" +
  ...
  board.prototype.createBoard = async function (param) {
    ...
    let insertBoardQuery =
      "INSERT INTO board (user_id,name,is_public,project_id)" +
      "VALUES(?,?,?,?)";
    return this.connection
        ...
        .then(function (data) {
          ...
          return self.connection
            ...
            .then(function (existingUser) {
              ...
              return Promise.all([
                addBoardUser(0, userToAddedd, boardId, self),
                deleteBoardUser(0, userToBeDeleted, boardId, self),
              ]).then(([value, rest]) => {
                let arr = [value, ...rest];
                let response = arr.reduce((e, j) => {
                  return Object.assign(e, j);
  ...
  board.prototype.deleteBoard = async function (param) {
    ...
    let { boardId } = param;
    let updateUserQuery =
      "UPDATE board SET is_deleted=1 WHERE id in (" + boardId + ")";
    let boardExists = await this.checkBoardExists(param.boardId);
    let userCanEdit = await this.checkUserIsAuthenticated(
      param.boardId,
      param.userId
    );
    if (boardExists && userCanEdit) {
  ...
  board.prototype.getAllBoard = async function (param) {
    let self = this;
    let res = new response();
    let { userId, projectId, itemLimit, offset, isActive } = param;
        ...
        return self.connection
          ...
          .then(async function (data2) {
            ...
            for (var x in boardResponse) {
              ...
              boardResponse[x].totalCompleteCard =
                totalCard[0].totalCompleteCard;
            }
  ...
  board.prototype.archivedBoard = async function (param) {
    let self = this;
    let res = new response();
    let { authenticateUserId, boardIds, projectId, archive } = param;
    // let hasBoard = await this.checkBoardExists(boardId);
    // let userRole = await this.checkUserRole(boardId, authenticateUserId);
    ...
    return this.connection
      ...
      .then(function (resp) {
        let query2 =
          "SELECT 'active' as type, COUNT(*) as total FROM board WHERE project_id = " +
          projectId +
          " AND is_archived = 0 UNION ALL SELECT 'archived' as type, COUNT(*) as total FROM board WHERE project_id = " +
          projectId +
          " AND is_archived = 1";
        return self.connection
          .query(self.connectionObject, query2)
          .then(function (resp2) {
            ...
            res.status = 200;
            res.data = Object.assign(resp, { total: resp2 });
            return res;
          })
          .catch(function (err) {
            console.log(err);
  ...
  board.prototype.getAllUser = async function (param) {
    ...
    return this.connection
      .query(
        this.connectionObject,
        "SELECT u.* FROM user u join board_user bu on bu.user_id = u.id  WHERE bu.board_id=" +
          boardId +
          ""
      )
      .then(function (data) {
  ...
  board.prototype.getTotalCard = async function (param) {
    ...
    const { boardId } = param;
    let query =
      "SELECT COUNT(card.id) AS totalCard, SUM(card.is_complete = 1) AS totalCompleteCard FROM card WHERE list_id IN (SELECT id FROM list WHERE board_id = " +
      boardId +
      ");";
    return this.connection
      .query(this.connectionObject, query)
