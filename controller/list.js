...
var listController = (function () {
  ...
  const updateCardPositionInList = function (idx, cards, type, self) {
    ...
    let query =
      type == "plus"
        ? "UPDATE card SET position=position+1 WHERE id=" + cards[idx] + ""
        : "UPDATE card SET position=position-1 WHERE id=" + cards[idx] + "";
    ...
    if (checkBoardExists) {
      ...
      if (
        ...
        let insertBoardQuery =
          ...
          "')";
        return this.connection
          .query(this.connectionObject, insertBoardQuery)
          .then(function (data) {
  ...
  list.prototype.editList = async function (param) {
    ...
    if (checkBoardExists && listExists) {
      ...
      if (
        ...
        if (!position) {
          insertBoardQuery =
            "UPDATE list SET name = '" + name + "' WHERE id=" + listId + "";
        }
        return this.connection
          .query(this.connectionObject, insertBoardQuery)
          .then(function (data) {
        ...
        .then(async function (data) {
          ...
          if (data.length > 0) {
            await data.forEach(async (e) => {
              ...
              if (
                !listObject[e.list_id].cards.hasOwnProperty(e.card_id) &&
                e.card_id != null
              ) {
              ...
              if (listObject[e.list_id].cards.hasOwnProperty(e.card_id)) {
                ...
                listObject[e.list_id].cards[e.card_id].description =
                  e.card_description;
                listObject[e.list_id].cards[e.card_id].create_date =
                  e.card_create_date;
                listObject[e.list_id].cards[e.card_id].isComplete =
                  e.card_is_complete;
                listObject[e.list_id].cards[e.card_id].position =
                  e.card_position;
                if (
                  !listObject[e.list_id].cards[e.card_id].hasOwnProperty(
                    "users"
                  )
                ) {
                ...
                if (
                  !listObject[e.list_id].cards[e.card_id].users.hasOwnProperty(
                    "e.card_user_id"
                  )
                ) {
                ...
                listObject[e.list_id].cards[e.card_id].users[
                  e.card_user_id
                ].email = e.card_user_email;

                // ADDING TAG IN THE CARD
                if (
                  !listObject[e.list_id].cards[e.card_id].hasOwnProperty("tags")
                ) {
                  listObject[e.list_id].cards[e.card_id].tags = {};
                }
                if (
                  !listObject[e.list_id].cards[e.card_id].tags.hasOwnProperty(
                    "e.tag_id"
                  )
                ) {
                  listObject[e.list_id].cards[e.card_id].tags[e.tag_id] = {};
                }
                listObject[e.list_id].cards[e.card_id].tags[e.tag_id].tagId =
                  e.tag_id;
                listObject[e.list_id].cards[e.card_id].tags[e.tag_id].tagName =
                  e.tag_name;
                listObject[e.list_id].cards[e.card_id].tags[e.tag_id].tagColor =
                  e.tag_color;
              }
            });
          }
    ...
    if (checkBoardExists) {
      ...
      if (
        ...
        return this.connection
          ...
          .then((data1) => {
            return this.connection
              .query(this.connectionObject, query2)
              .then((data2) => {
                res.message = "Card has been updated";
                res.status = 200;
  ...
  list.prototype.updateCardPositionInList = async function (param) {
    ...
    const {
      authenticatedUser,
      boardId,
      cardsPositionToBePlus,
      cardsPositionToBeMinus,
    } = param;
    var checkBoardExists = await this.board.checkBoardExists(boardId);
    if (checkBoardExists) {
      var userIsAuthenticated = await this.board.checkUserIsAuthenticated(
      ...
      if (
        ...
        return Promise.all([
          updateCardPositionInList(0, cardsPositionToBePlus, "plus", this),
          updateCardPositionInList(0, cardsPositionToBeMinus, "minus", this),
        ]).then(([value, rest]) => {
          let arr = [value, ...rest];
          let response = arr.reduce((e, j) => {
            return Object.assign(e, j);
  ...
  list.prototype.archivedList = async function (param) {
    ...
    if (checkBoardExists) {
      let query =
        "UPDATE list SET is_archived=" + archived + " WHERE id=" + listId + "";
      try {
        return this.connection
          .query(this.connectionObject, query)
          .then((data) => {
