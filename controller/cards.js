...
var cardController = (function () {
  ...
  function addCardActivity(cardId, userId, activityObject, self) {
    ...
    let query =
      "INSERT INTO card_activity (card_id, user_id,activity) VALUES(" +
      cardId +
      ", " +
      userId +
      ", '" +
        ...
        if (activityObject.hasUser) {
          await self.connection
            .query(
              ...
              "SELECT created_date FROM card_activity WHERE id = " +
                data3.insertId +
                ""
            )
            .then(async function (data4) {
              ...
              await self.connection
                .query(
                  self.connectionObject,
                  "INSERT INTO card_activity_added_user (card_activity_id,added_user_id,type,created_date) VALUES(" +
                    data3.insertId +
                    "," +
                    activityObject.userId +
                    "," +
                    activityObject.type +
                    ", '" +
                    toMySQLDateTime(data4[0].created_date) +
                    "')"
                )
          ...
          await self.connection
            ...
            .then(async function (data4) {
              ...
              await self.connection
                .query(
                  self.connectionObject,
                  "INSERT INTO `card_activity_added_checklist` (card_activity_id,added_checklist_id,type,created_date) VALUES(" +
                    data3.insertId +
                    "," +
                    activityObject.checklistId +
                    "," +
                    activityObject.type +
                    ", '" +
                    toMySQLDateTime(data4[0].created_date) +
  ...
  function addUserToCard(idx, users, cardId, loggedInUser, self) {
    ...
    let query =
      "INSERT INTO card_user (user_id,card_id,role_id)" +
      "VALUES('" +
      userID +
      "','" +
      cardId +
    ...
    const p1 = new Promise((resolve, reject) => {
      self.connection
        ...
        .then(async function (data3) {
          let activityObject = {
            activity: activity.CARD_ADD_USER,
            hasUser: true,
            userId: userID,
            type: 1,
          };
  ...
  const deleteUserToCard = function (idx, users, cardId, loggedInUser, self) {
    ...
    let userID = users[idx].user_id || users[idx].id;
    let insertBoardUserQuery =
      "DELETE FROM card_user WHERE card_id= '" +
      cardId +
      "' AND user_id = '" +
      userID +
      "'";
    let pr = new Promise((resolve, reject) => {
      self.connection
        .query(self.connectionObject, insertBoardUserQuery)
        .then(async function (data3) {
          data3.cardId = cardId;
          let activityObject = {
            activity: activity.CARD_REMOVE_USER,
            hasUser: true,
            userId: userID,
            type: 0,
          };
  ...
  const updatePosition = async function (idx, cards, self) {
    ...
    const pr1 = new Promise((resolve, reject) => {
      try {
        self.connection.query(self.connectionObject, query).then((data) => {
  ...
  const checkUserHasPermission = async function (cardId, userId, self) {
    let query =
      "SELECT COUNT(*) as totalCards FROM card_user cu WHERE cu.card_id=" +
      cardId +
      " AND cu.user_id=" +
      userId +
      "";
    return self.connection
      .query(self.connectionObject, query)
      .then(function (data) {
        if (data[0].totalCards > 0) return true;
        else return false;
      })
      .catch(function (err) {
        console.log(err);
  ...
  card.prototype.checkTagExistsInCard = function (tag, cardId) {
    return this.connection
      .query(
        this.connectionObject,
        "SELECT * FROM card_tag WHERE tag='" +
          tag +
          "' AND card_id='" +
          cardId +
          "'"
  ...
  card.prototype.checkUserRole = async function (cardId, userId) {
    let query =
      "SELECT DISTINCT r.role AS role_name " +
      "FROM card_user cu " +
  ...
  card.prototype.createCards = async function (param) {
    ...
    let insertBoardQuery =
      "INSERT INTO card (list_id,user_id, name, description,is_active, due_date, reminder_date, position) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    return this.connection
      .queryByArray(this.connectionObject, insertBoardQuery, [
        listId,
        userId,
        name,
        description || null,
        ...
        return self.connection
          ...
          .then(function (data2) {
            ...
            data2.forEach(function (e) {
              if (e.role == "ROLE_SUPER_ADMIN") roleId = e.id;
            });
            let insertCardUserQuery =
              "INSERT INTO card_user (user_id,card_id,role_id)" +
              "VALUES('" +
              userId +
              "','" +
              ...
              "')";
            return self.connection
              .query(self.connectionObject, insertCardUserQuery)
              .then(async function (data3) {
                console.log(data3);
  ...
  card.prototype.editCards = async function (param) {
    ...
    var checkListExists = await this.checkListExists(listId);
    if (checkBoardExists && checkListExists) {
      var userIsAuthenticated = await this.board.checkUserIsAuthenticated(
        boardId,
        userId
      );
      var userRole = await this.board.checkUserRole(boardId, userId);
      if (
        userIsAuthenticated &&
        userRole.length > 0 &&
        (userRole[0].role_name == "ROLE_SUPER_ADMIN" ||
  ...
  card.prototype.setCardStatus = async function (param) {
    ...
    if (checkBoardExists && checkListExists) {
      ...
      if (
        ...
        let insertBoardQuery =
          "UPDATE card" + " SET is_complete = ?" + " WHERE id = ?";
        return this.connection
          .queryByArray(this.connectionObject, insertBoardQuery, [
            isComplete,
            cardId,
          ...
          .then(async function (data) {
            console.log(data);
            const activitiText = isComplete
              ? activity.CARD_STATUS_COMPLETED
              : activity.CARD_STATUS_INCOMPLETE;
  ...
  card.prototype.addUsers = async function (param) {
    ...
    if (
      ...
      userRoleForBoard[0].role_name == "ROLE_SUPER_ADMIN" &&
      userRole.length > 0 &&
      (userRole[0].role_name == "ROLE_SUPER_ADMIN" ||
        userRole[0].role_name == "ROLE_ADMIN")
    ) {
      let selectExistingQuery =
        "SELECT user_id,role_id from card_user WHERE card_id='" +
        cardId +
        "' AND NOT user_id='" +
        authenticateUserId +
        "' ";
      return self.connection
        ...
        .then(function (existingUser) {
          ...
          return Promise.all([
            addUserToCard(0, userToAddedd, cardId, authenticateUserId, self),
            deleteUserToCard(
              0,
              userToBeDeleted,
              cardId,
            ...
            if (response.length == 0) return [{ status: 200 }];
            return [...response];
          });
        });
    } else {
  ...
  card.prototype.deleteCards = async function (param) {
    ...
    let updateUserQuery = "DELETE FROM card " + "WHERE id='" + cardId + "'";
    ...
    if (boardExists && listExists && userCanEdit) {
      return this.connection
        .query(this.connectionObject, query1)
        .then(function (data) {
          if (boardExists && listExists && userCanEdit) {
            return self.connection
              .query(self.connectionObject, updateUserQuery)
              .then(function (data2) {
                res.message = "Card Has been deleted";
                res.status = 200;
                res.data = data2;
  ...
  card.prototype.updateIsDeleteInCard = async function (param) {
    var self = this;
    let res = new response();
    let { boardId, listId, cardId } = param;
    let query1 = "UPDATE card SET is_deleted=1 WHERE id = '" + cardId + "'";
    //let updateUserQuery = "DELETE FROM card " + "WHERE id='" + cardId + "'";
    let boardExists = await this.board.checkBoardExists(boardId);
    let listExists = await this.checkListExists(listId);
    let userCanEdit = await this.board.checkUserIsAuthenticated(
  ...
  card.prototype.getAllCards = async function (param) {
    ...
    if (boardExists) {
      ...
      if (userIsAuthenticated) {
        let query =
          "SELECT c.* FROM card c join card_user cu on cu.card_id = c.id " +
          "where c.list_id = " +
          listId +
          " AND c.is_deleted=0 " +
          " and cu.user_id = " +
        ...
        .then(function (data) {
          ...
          data.forEach((e) => {
            ...
            cardObject[e.id].checkList = cardObject[e.id].hasOwnProperty(
              ...
              ? cardObject[e.id].checkList
              : [];
            if (!checkListSet.has(e.cli_id)) {
              if (e.cli_id) {
                cardObject[e.id].checkList.push({
                  cliId: e.cli_id,
                  cliName: e.cli_name,
                  cliPosition: e.cli_position,
                  cliIsChecked: e.cli_isChecked,
                });
                checkListSet.add(e.cli_id);
              }
            }
            // ADD COMMENT
            cardObject[e.id].comments = cardObject[e.id].hasOwnProperty(
              "comments"
            )
              ? cardObject[e.id].comments
              : [];
            if (!commentSet.has(e.commentId)) {
              if (e.commentId) {
                cardObject[e.id].comments.push({
                  id: e.commentId,
                  user: e.cmntUser2,
                  date: e.commentDate,
                  comment: e.comment,
  ...
  card.prototype.addCheckListItem = async function (params) {
    let self = this;
    let res = new response();
    let { authenticateUserId, cardId, boardId, name, isChecked, position } =
      params;
    let hasUser = await this.user.checkUserExistsById(authenticateUserId);
    let userRoleForBoard = await this.board.checkUserRole(
      boardId,
    ...
    if (
      ...
      let query =
        "INSERT INTO `checklist_item` (card_id,name,is_checked,position) VALUES('" +
        cardId +
        "','" +
        name +
        "','" +
      ...
      return self.connection
        ...
        .then(async function (data) {
          const activityObject = {
            activity: activity.CARD_CHECKLIST_ADDED,
            hasChecklist: true,
            type: 1,
            checklistId: data.insertId,
          };
          await addCardActivity(
            cardId,
            authenticateUserId,
  ...
  card.prototype.editCheckListItem = async function (params) {
    let self = this;
    let res = new response();
    let { authenticateUserId, cardId, boardId, id, name, isChecked, position } =
      params;
    let hasUser = await this.user.checkUserExistsById(authenticateUserId);
    let userRoleForBoard = await this.board.checkUserRole(
      boardId,
    ...
    if (
      ...
      let query =
        "UPDATE `checklist_item` set name='" +
        name +
        "', is_checked='" +
        isChecked +
        "', position='" +
      ...
      return self.connection
        ...
        .then(async function (data) {
          const activityObject = {
            activity: activity.CARD_CHECKLIST_EDITED,
            hasChecklist: true,
            type: 1,
            checklistId: id,
          };
  ...
  card.prototype.deleteCheckListItem = async function (params) {
    ...
    if (
      ...
      return self.connection
        ...
        .then(async function (data) {
          const activityObject = {
            activity: activity.CARD_CHECKLIST_DELETE,
            hasChecklist: true,
            type: 0,
            checklistId: null,
          };
  ...
  card.prototype.createComments = async function (params) {
    ...
    if (checkBoardExists && checkListExists) {
      ...
      if (userIsAuthenticated && userRole.length > 0) {
        let query =
          "INSERT INTO comment (card_id,user_id,comment,created_date) VALUES ('" +
          cardId +
          "', '" +
          authenticateUserId +
          "','" +
  ...
  const addMultipleTags = async function (
    idx,
    tags,
    userId,
    cardId,
  ...
  const addMultipleChecklist = async function (
    idx,
    checkList,
    userId,
    cardId,
    ...
    const p1 = new Promise(async (resolve, reject) => {
      try {
        self
          .addCheckListItem({
            authenticateUserId: userId,
            name: checkList[idx].cliName,
            isChecked: checkList[idx].cliIsChecked,
            position: checkList[idx].cliPosition,
            cardId,
            boardId,
          })
          .then(async (e) => {
            await addMultipleChecklist(
              idx + 1,
              checkList,
              userId,
              cardId,
  ...
  const copyCardHelper = async function (
    idx,
    listIds,
    userIds,
    tagIds,
    ...
    try {
      const addedCard = await self.createCards({
        userId,
        name: card.data[Object.keys(card.data)[0]].name,
        listId: listIds[idx],
        description: card.data[Object.keys(card.data)[0]].description,
        isActive: card.data[Object.keys(card.data)[0]].complete,
        dueDate: card.data[Object.keys(card.data)[0]].reminder_date,
        ...
        const p1 = new Promise(async (resolve, reject) => {
          self
            .addUsers({
              authenticateUserId: userId,
              users: userIds,
              cardId: addedCard.data.lastInsertCardId,
              boardId,
            })
            .then(async (e) => {
              ...
              if (e[0].status && e[0].status == 200) {
                ...
                if (tagIds && tagIds.length > 0) {
                  addMultipleTags(
                    ...
                    tagIds,
                    userId,
                    addedCard.data.lastInsertCardId,
                    boardId,
                    self
                  ).then(async (e2) => {
                    console.log("TAG========");
  ...
  card.prototype.copyCard = async function (params) {
    ...
    if (checkBoardExists && checkListExists) {
      ...
      if (
        ...
        let card = await this.getCardById({
          boardId,
        ...
        return copyCardHelper(
          0,
          listIds,
          card.data[cardId].users,
          card.data[cardId].tags,
          card.data[cardId].checkList,
          authenticateUserId,
          card,
          boardId,
          self
