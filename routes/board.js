 ...
 */
router.get(
  "/getAllBoard/:projectId/:isActive/:itemLimit/:offset",
...
router.get("/getAllBoardTag/:boardId", async function (req, res) {
  let { boardId } = req.params;
  ...
  if (!req.authenticatedUser || !boardId) {
    ...
    try {
      var response = await board.getAllTag(req.params);
      console.log(response);
...
router.put("/editBoardTag", async function (req, res) {
  let { boardId, projectId, tagId, name, color, attachInBoard } = req.body;
  Object.assign(req.body, { authenticateUserId: req.authenticatedUser.id });
  if (
    !req.authenticatedUser ||
    !boardId ||
    ...
    !color ||
    !tagId ||
    attachInBoard == undefined
  ) {
    res.status(400).send(new error("Send Proper data."));
    return;
  } else {
    try {
      var response = await board.editBoardTag(req.body);
      res.status(response.status).send(response);
    } catch (err) {
      console.log(err);
...
router.post("/addBoardTag", async function (req, res) {
  let { boardId, projectId, name, color, attachInBoard } = req.body;
  Object.assign(req.body, { authenticateUserId: req.authenticatedUser.id });
  if (
    !req.authenticatedUser ||
    !boardId ||
    ...
    try {
      var response = await board.addBoardTag(req.body);
      res.status(response.status).send(response);
    } catch (err) {
      console.log(err);
...
router.delete(
  "/deleteBoardTag/:boardTagId/:boardId",
  async function (req, res) {
    let { boardId, boardTagId } = req.params;
    Object.assign(req.body, { authenticateUserId: req.authenticatedUser.id });
    if (!req.authenticatedUser || !boardId || !boardTagId) {
      res.status(400).send(new error("Send Proper data."));
      return;
    } else {
      try {
        var response = await board.deleteBoardTag(req.params);
        res.status(response.status).send(response);
      } catch (err) {
        console.log(err);
