 ...
 *      500:
 */
router.get("/getAllList/:boardId/:isArchive", async function (req, res) {
  let { boardId, isArchive } = req.params;
  Object.assign(req.params, { userId: req.authenticatedUser.id });
  if (!req.authenticatedUser || !boardId || isArchive == undefined) {
    res.status(400).send(new error("Send Proper data."));
    return;
  } else {
    try {
...
router.put("/updateCardPosition", async function (req, res) {
  let { boardId, cardsPositionToBePlus, cardsPositionToBeMinus, listId } =
    req.body;
  Object.assign(req.body, { authenticatedUser: req.authenticatedUser.id });
  console.log(req.authenticatedUser);
...
router.put("/archived", async function (req, res) {
  ...
  if (!req.authenticatedUser || !boardId || archived == undefined || !listId) {
    res.status(400).send(new error("Send Proper data."));
    return;
  } else {
    try {
